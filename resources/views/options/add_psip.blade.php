...
@include('layouts.partials.messages')
    <div class="bg-light p-5 rounded">
        ...
        <h1>Add a new PSIP to the Database</h1>
    ...
    <div class="bg-light p-4 rounded">
        ...
        <div class="container mt-4">
            ...
            <form method="POST" action="{{ route('psip.store') }}" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label for="group" class="form-label">Project Group</label>

                    <select id="group" name="group" class="form-control dropdown" required>
                        <option disabled="" selected=""></option>
                        @foreach($groups as $group)
                        <option value="{{$group->id}}" {{old('group')==$group->id ? 'selected' : '' }}>{{$group->group_name}} - {{$group->subitem()->first()->sub_name}}</b></option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select id="division" name="division" class="form-control dropdown" required>
                        <option disabled="" selected=""></option>
                ...
                <div class="mb-3">
                    ...
                    <textarea value="{{ old('description') }}"
                        ...
                        class="form-control"
                        name="description"
                        placeholder="Drag lower right hand corner to expand."></textarea>
