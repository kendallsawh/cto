    ...
    <div class="bg-light p-4 rounded">
        ...
        <div class="container mt-4">
            ...
            <form method="POST" action="{{ route('psip.update', $psip->id) }}" enctype="multipart/form-data">
                ...
                @php
                    $financials = $psip->psipDetailForCurrentYear ? $psip->psipDetailForCurrentYear->psipFinancials : [];
                @endphp
                ...
                @if (!empty($financials))
                    ...
                    @endforeach
                @else
                    {{-- No data available, show empty fields for user to input --}}
                    <!-- <div class="financial-section mb-4">
                        <div class="form-group">
                            <label for="actual_expenditure">Actual Expenditure</label>
                            <input type="number" step="0.01" class="form-control" id="actual_expenditure" name="actual_expenditure[]" value="">
                        </div>

                        <div class="form-group mt-2">
                            <label for="approved_estimates">Approved Estimates</label>
                            <input type="number" step="0.01" class="form-control" id="approved_estimates" name="approved_estimates[]" value="">
                        </div>
                    ...
                    <strong class="form-text text-danger">
                            No financial details available for the current year. Please contact Agricultural Planning to update this PSIP to the current year.
                        </strong>
                @endif
                ...
                <h6>Activities</h6>
                @if($psip->activitiesForCurrentYear->count())
                @foreach ($psip->activitiesForCurrentYear as $activity)
                ...
                <div class="form-group">
                    ...
                    <strong class="form-text text-danger">
                        No activity details available for the current year. Please contact Agricultural Planning to update this PSIP to the current year.
                    </strong>
                </div>
                @endif
